...
BUTTON_CONFIRM_RESET_STATS = "🔄 Сбросить"
BUTTON_CONFIRM_DELETE_OPPONENT = "❌ Удалить"
...
def score_prompt(opponent_name: str) -> str:
    return (
        ...
        "<hr/>"
        "<blockquote>"
        "Напиши два числа в одном сообщении: сначала свой счёт, потом — соперника. Например: <code>11-7</code> или <code>15 13</code>."
        "</blockquote>"
    )
...
def format_stats(
    ...
    return (
        ...
        f"<tr><td align=\"left\">Всего игр</td><td colspan=\"2\" align=\"left\">{stats.games}</td></tr>"
        f"<tr><td align=\"left\">Мячи</td><td align=\"left\">{stats.points_for} ({points_difference})</td>"
        f"<td align=\"left\">{stats.points_against}</td></tr>"
        f"<tr><td align=\"left\">Всего мячей</td><td colspan=\"2\" align=\"left\">"
...
def format_extended_stats_rows(extended_stats: Optional[ExtendedStatsLike]) -> str:
    ...
    return (
        ...
        f"<tr><td align=\"left\">Всего овертаймов</td><td colspan=\"2\" align=\"left\">"
